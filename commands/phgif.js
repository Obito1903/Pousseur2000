...
exports.run = (client, message, args) => {
    if (!args[0]) return message.channel.send(`Il n'y a pas d'argument. ${client.config.prefix}help pour plus d'info.`)
    ...
    Pornsearch.search(args[0])
        .gifs(Math.floor(Math.random() * 5))
        .then(gifs => {
